...
class TileEncoderActor:
    def __init__(self, DEVICE: torch.device, MODEL_DTYPE: torch.dtype, STAIN_VECTORS: np.array):
        ...
        self.tile_encoder = model.to(self.device).to(self.model_dtype)
    ...
    def __call__(self, batch: pd.DataFrame) -> pd.DataFrame:
        ...
        with torch.inference_mode():
            output = self.tile_encoder(final_input_tensor)

            class_token = output[:, 0]    # size: 1 x 1280
            patch_tokens = output[:, 5:]  # size: 1 x 256 x 1280, tokens 1-4 are register tokens so we ignore those

            # concatenate class token and average pool of patch tokens
            embeddings_tensor = torch.cat([class_token, patch_tokens.mean(1)], dim=-1)
...
if __name__ == "__main__":
    # Path to the library that fix 'jpeg12' error
